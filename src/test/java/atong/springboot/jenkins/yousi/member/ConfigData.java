package atong.springboot.jenkins.yousi.member;

public class ConfigData {
    /*地址集合列表*/
    public static String vipHomePageUrl = "/api/v1.0/umsVipMember/homePage";
    public static String memOrderUrl = "/api/v1.0/pay/rechargeOrStoreMember";
    /*Content-Type设置*/
    public static String ContentType = "application/json";
    public static final String DEVICEID = "A5A6F0c6B90638A2F-e195d43830A5e9979906e5A0A8A-9330A0B3ADBBB9d93-AFF5dBcF9-A4c749-AB10-4EB49EABF9E7-85315174-34961239";
    public static final String DEVICESOURCE = "native";
    public static final String lang = "en-US";
    public static final String COOKIE = "r=f98cef43-5dc5-48a0-8c99-1c62f549d511";

}
